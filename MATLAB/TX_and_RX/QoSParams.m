...
classdef QoSParams < handle
    ...
    methods (Access = public)
        ...
        function req_power_W = reqTXPower(this, channel_gain_dB)
            ...
            req_power_W = this.calcLinPower(1./CNR_lin);
        end
        
        function gamma_th = thresholdChannelGain(this, txpwr_W)
            CNR_lin = this.calcLinCNR(txpwr_W);
            gamma_th = this.calcChannelPowerdBm(CNR_lin);
        end
        ...
        function exp_req_power_W = expReqTXPowerW(this, exp_ch_gain_dB, exp_ch_gain_var)
            ...
            exp_req_power_W = this.calcLinPower(exp_CNR_lin_inverse);
        end
    ...
    methods (Access = private)
        ...
        function power_W = calcLinPower(this, CNR_lin_inverse)
            %divide by 1000 to get Watts, not milliwatts
            power_W = (((2^this.R - 1)/this.K)*CNR_lin_inverse)/1000;
        end
        
        function CNR = calcLinCNR(this, power_W)
            CNR_inv = power_W*1000/((2^this.R - 1)/this.K);
            CNR = 1/CNR_inv;
        end
        
        function gamma_dBm = calcChannelPowerdBm(this, CNR_lin)
            gamma_dBm = 10*log10(this.RXNoise*CNR_lin);
        end
    end
end
