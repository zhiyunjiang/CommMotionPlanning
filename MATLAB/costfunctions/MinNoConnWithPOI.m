...
function total_cost = MinNoConnWithPOI(path, bs_cc, poi_cc, gamma_th_dBm, eps, scenario)
    ...
    bs_conn_a = ( bs_cc.getGammaTOTdBAtPt(path(1,:)) >= gamma_th_dBm);
    poi_conn_a = ( poi_cc.getGammaTOTdBAtPt(path(1,:)) >= gamma_th_dBm);
    for i=2:length(path)
        bs_conn_b = ( bs_cc.getGammaTOTdBAtPt(path(i,:)) >= gamma_th_dBm) ;
        poi_conn_b = ( poi_cc.getGammaTOTdBAtPt(path(i,:)) >= gamma_th_dBm) ;
