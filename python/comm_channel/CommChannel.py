...
class CommChannel:
        ...
        def sampleChannel(self, n):
                ...
                """
                xs = np.random.randint(self.xImax, size = n )
                ys = np.random.randint(self.yImax, size = n )
                grid_pts = np.array(list(zip(xs, ys)))
                sample_pos = toRawFromGrid(self.region, self.res, grid_pts)
                sample_vals = self._getGammaTOTdBAtGridPoint(grid_pts)
